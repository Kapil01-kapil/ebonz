...
import { postActions, favoriteActions, conversationActions } from '../../../Actions';
...
class AddDetails extends Component {
  ...
  componentDidMount() {
   ...
   const isLiked=this.props.navigation.getParam('isLiked','');
  ...
  chat() {
    ...
    const friendId = this.state.postdata.user._id;
    const { dispatch, user } = this.props;
    
    const data={postId:post._id,
    sellerId:post.user._id,
    buyerId:user.id
    }
    dispatch(conversationActions.getConversationByIds(user.id,post._id,post.user._id,user.id))
    // dispatch(conversationActions.getMyConversations(user.id));
     this.props.navigation.navigate(Routes.Chats, { friendId: friendId });
  }
  sendReview() {
  ...
  addToFav() {
   ...
   // const post=this.props.data;
    ...
    // if (!isLiked) {
    //   dispatch(favoriteActions.add({ userId: user.id, postId: post }));
  ...
  render() {
    ...
    if (postdata) {
     ...
     // console.log(postItems);
      ...
      details = parameters && Object.keys(parameters).map((key, i) => {
        return (
          <View
            style={{
      ...
      slides2 =
        ...
        postdata.videos.map((video, i) => {
          return (
            <View style={{ height: 200, flex: 1 }} key={i}>
              <Video
          ...
          <Container >
            <ScrollView>
              <View style={{ padding: 10 }}>
                <View>
                  <View>
                    <View style={{ paddingVertical: 5 }}>
                      <Text style={{ fontSize:HeadingFont, color: colors.FOREGROUND_DARK }}>{postdata.title}</Text>
                    </View>
                    <View style={{ flex: 1, flexDirection: 'row' }}>
                      <View style={{ flex: 3 }}>
                        <Text style={{ fontSize:MediumFont, color: colors.HEADER }}>
                ...
                <View >
                  <View style={{ alignItems: "flex-end", top: 10, paddingRight: 5 }}>
                    <View style={{ flex: 1, flexDirection: 'row' }}>
                      <View style={{ flexDirection: 'row', marginRight: 5 }}>
                        <Icon
                          ...
                          size={iconsize}
                        />
                        <Text style={{ fontSize: 16, color: colors.FOREGROUND_DARK, paddingHorizontal: 5, paddingVertical: 7 }}>{postdata.reviews}</Text>
                      </View>
                      <TouchableOpacity>
                        <View style={{ flex: 1, flexDirection: 'row' }}><Icon
                          color={isLiked ? colors.HEADER : colors.BASE_COLOR}
                          onPress={() =>
                            this.addToFav()
                          } name="ios-thumbs-up"
                          size={iconsize}
                        />
                          <Text style={{ fontSize: 16, color: colors.FOREGROUND_DARK, paddingHorizontal: 5, paddingVertical: 7 }}>{postdata.likes}</Text>
                        </View>
                      </TouchableOpacity>
                    </View></View>
                </View>
                <View
                  ...
                  <Text style={{ fontSize:NormalFont, marginLeft: 10, color: colors.FOREGROUND_DARK }}>
                ...
                <View style={styles.detail}>
                  ...
                  <Text
                    style={{ fontSize:MediumFont, color: colors.FOREGROUND_DARK }}>
                    {postdata.description}
                  </Text>
                </View>
                {details && (
                  <View >
                    <Text style={{ fontSize:HeadingFont, color: colors.FOREGROUND_DARK, paddingVertical: 10 }}>Details</Text>
                    <View
                      style={{
                        justifyContent: 'space-between',
                        alignItems: 'center',
                ...
                {(postdata.user._id != user.id) ? <TouchableOpacity
                  onPress={() => {
                    // this.props.navigation.navigate(Routes.UserInfo);
                    this.props.navigation.navigate(Routes.UserInfo, {
                      postuser: postdata.user,
                    });
                  }}>
      {user.profile&&            <View style={[styles.view1, { borderTopWidth: 1, borderBottomWidth: 1, borderColor: colors.HEADER }]}>

                    <View style={styles.man}>
                      <Image
                        source={{ uri: postdata.user.profile.thumbnail }}
                        style={{ height: 70, width: 70, borderRadius: 70, resizeMode: 'cover' }}>

                      </Image>
                    </View>
                    <View style={{ marginLeft: 20 }}>
                      <Text style={{ color: colors.HEADER, fontSize: 20 }}>
                        {postdata.user.profile.name ||
                          postdata.user.profile.mobile ||
                          postdata.user.profile.email}
                      </Text>
                    </View>
                  </View>}
                ...
                <View
                  ...
                  <Text
                    style={{ fontSize:HeadingFont, color: colors.FOREGROUND_DARK }}>
                    Ad Posted at
                </Text>
                  ...
                  <Image
                    source={{
                      uri:
                        'https://maps.googleapis.com/maps/api/staticmap?zoom=13&size=800x400&maptype=roadmap&markers=color:blue%7Clabel:S%7C' +
                ...
                <View
                  ...
                  <View style={{height:20}}><Text style={{ fontSize:NormalFont, color: 'black',color: colors.FOREGROUND_DARK }}>
                    AD ID: {postdata._id}
                  </Text></View>
                  {/* <Text style={{fontSize: 15, color: '#007ACC'}}>
                  REPORT THIS AD
                </Text> */}
                </View>
                ...
                <View
                  ...
                  <View
                    ...
                    <TouchableOpacity onPress={() => this.sendReview()}>
                      <View style={{ backgroundColor: colors.HEADER, paddingVertical: 5, paddingHorizontal: 10, borderRadius: 5, margin: 1 }}>
                        {/* <Text style={{color: 'white'}}>Send</Text> */}
                        <Icon name="ios-arrow-round-forward" size={40} color={colors.FOREGROUND_LIGHT}></Icon>
                      </View>
                    </TouchableOpacity>
                  </View>
                  {(postdata.user._id != user.id) ? <View
                    style={{
                      flex: 1,
                      marginTop: 10,
                      flexDirection: 'row',
                    ...
                    <TouchableOpacity
                      onPress={() => this.call(postdata.user.profile)}
                      style={[styles.button, { backgroundColor: colors.HEADER }]}>
                      <Icon
                        style={{ marginLeft: 10 }}
                        color="white"
                ...
                onPress={() => {
                  ...
                  setTimeout(() => {
                    if (typeof shareOptions['url'] !== undefined) {
                      ...
                      Toast.show('Link copied to clipboard', Toast.LONG);
...
const connectedAddDetailsPage = connect(mapStateToProps)(AddDetails);
export { connectedAddDetailsPage as AddDetails };
